...
- 基于 v4.02.30 相同修复代码发布新版本，用于验证 v4.02.30 → v4.02.31 的自动强制更新完整流程。
- 保持 `force_update=true`，用于确认更新提示、下载校验、安装包启动均可正常执行。
...
- 修复强制更新流程关闭启动窗口后触发 Qt 自动退出、导致尚未下载更新包就“闪退”的问题。
- 启动、更新和登录窗口切换期间禁用“关闭最后窗口即退出”，主窗口显示后恢复正常关闭行为。
- 更新提示阶段隐藏启动页而不是关闭启动页，避免触发最后窗口关闭事件。
- 新增应用窗口生命周期回归测试。
...
- 默认选择伊起牛登录。
- 伊起牛与原账号登录均可选择“记住密码”。
- 密码只保存到 macOS Keychain 或 Windows 凭据管理器，普通配置文件仅保存非敏感登录偏好。
- 旧版本地加密凭据首次读取后迁移到系统凭据库，并清理旧密文和本地密钥。

### 更新与版本

- 主界面顶部显示当前版本并增加“检查更新”入口。
- 手动检查发现强制更新后不可跳过；下载或启动失败时只能重试或退出。
- 保留启动阶段联网强制检查，检查失败时不允许进入软件。

### 界面显示

- 重做界面显示设置，提供紧凑、推荐、舒适及自定义方案。
- 增加主题选择和实时预览，限制到经过验证的缩放和字号范围。
- 修复 Qt 已处理系统 DPI 后再次乘算造成的重复放大。
- 重做启动更新页，解决高分屏下图标、标题、版本、状态和进度条重叠问题。

### 验证

- 通过 24 项认证、更新、计算口径和界面布局测试。
- 完成启动页 100%、125%、150% 缩放离屏渲染检查。
