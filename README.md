...
- **认证服务**：HTTPS API - 原账号/伊起牛双登录、邀请码注册和当前账号改密
...
- **用户认证**：桌面端仅通过 HTTPS 认证 API 访问账号服务，不包含数据库凭据
- **密码管理**：原账号首次登录必须改密，且仅能修改当前账号；忘记密码需管理员核验身份
- **数据加密**：本地凭证使用Fernet加密存储
- **软件更新**：Windows/macOS 安装包通过固定 OSS 地址分发，并校验文件大小和 SHA-256
